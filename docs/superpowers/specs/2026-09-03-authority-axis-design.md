...
### 4.4 `statutes.ts` — 참조 법령

앱은 이미 화면에서 법적 사실을 단정하고 있었다. 근거가 없었을 뿐이다.

| 앱의 기존 주장 | 위치 | 근거 |
|---|---|---|
| 후견계약은 공정증서로 | `guardianship.ts:291` | 민법 §959-14② |
| 감독인 선임으로 효력 발생 | `guardianship.ts:312` | 민법 §959-14③ |
| 등기가 선행되어야 함 | `guardianship.ts:420` | 민법 §959-15① |
| 성년 / 한정 / 특정후견 | `guardianship.ts` 판정 트리 | 민법 §9① / §12① / §14-2①② |
| 신탁 설정 방법 | `trust.ts` | 신탁법 §3① |
| 유언대용 / 수익자연속 | `trust.ts` `decideType()` | 신탁법 §59② / §60 |

원칙 셋.

1. **조문 전문을 그대로 싣는다.** 요약하면 그 요약이 우리 주장이 되고 인용이 아니게 된다.
2. **구조적이고 오래 안 바뀐 조문만 넣는다.** 유류분(민법 §1112 이하)은 넣지 않는다 —
   2024년 헌법재판소 결정으로 내용이 달라진 부분이 있어 현행과 어긋날 위험이 크다.
3. **명문 근거가 없는 것에는 붙이지 않는다.** "의사능력 없는 자의 법률행위는 무효"는 판례
   법리이고 민법에 그 문언이 없다. 금융기관 대리인 지정도 약관·내규의 영역이다.
   **번호를 붙이는 순간 그것이 틀린 인용이 된다. 얕은 인용은 인용이 없는 것보다 나쁘다.**

부수 효과로 §959-15① 에서 청구권자를 확인해, 보호자 안내가 "보호자 또는 법정 청구권자"
에서 **"본인, 배우자, 4촌 이내의 친족, 검사 또는 지방자치단체의 장"** 으로 구체화되었다.

확인일 2026-09-03. 고지에 그 시점과 개정 가능성을 함께 싣는다.
...
```
① 치매 진단 → 신탁 제4조 지급개시 트리거
   🔒 집행 근거 없음
      「치매대비 자익신탁」이 아직 AI 초안 상태입니다.
      효력 발생: 신탁계약을 체결하고 신탁재산의 이전을 마친 때
      [→ 전문가에게 전달할 의뢰서 만들기]
```

**구현해 보니 ①부터 잠긴다.** 스펙 초안에는 "③에서 자물쇠"라고 적었으나, 신탁계약이 없으면
제4조 트리거 자체가 성립하지 않으므로 조항이 걸린 모든 단계가 잠긴다. 논리적으로 옳고,
메시지는 오히려 더 세다 — **설계서만 있으면 아무 일도 일어나지 않는다.**

상단에 문서별 체결 토글을 둔다. 발표 장면은 두 박자다.

```
1박  전부 초안       → ①②③④⑤ 전부 자물쇠. 완성도 91% 인데 집행 0건
2박  신탁만 체결     → ①②④⑤ 흐름. ③은 반만 잠김 —
                       신탁 제5조는 지급되지만 expense §2 자동이체는 위임장이 없어 수동
```

2박의 ③이 이 축의 정밀도를 보여준다. 문서마다 `covers` 범위가 다르므로 **한 노드 안에서도
조항별로 잠김이 갈린다.** 잠긴 조항만 `locked: true` 가 붙고 나머지는 그대로 집행된다.

토글의 의미를 화면에 명시한다 — *"체결 여부는 이 화면에서 정해지지 않습니다. 공증·등기·법원
심판·금융기관 등록처럼 앱 바깥에서 끝난 일을 여기에 알려주는 것입니다."*
...
| `applyAuthority` | 신탁만 체결했을 때 한 노드 안에서 조항별로 잠김이 갈리는지 | `covers` 범위 분리가 실제로 작동하는지 |
| `applyAuthority` | 원본 `ScenarioResult` 를 변형하지 않는지 | 후처리가 원본을 오염시키면 재계산이 어긋난다 |
| `statutesFor` | 유류분 조문이 들어가지 않는지 | 2024년 헌재 결정으로 변동 |
| `statutesFor` | 근거가 없는 문서(`bank_mandate`)에 조문을 달지 않는지 | 지어낸 인용이 가장 큰 감점 |
...
3. **게이팅** — `applyAuthority`, `/simulation` 확장, 체결 토글 ← **데모 하이라이트**
4. **서술층** — `/api/ai/referral`, §1 의뢰 취지와 §4 미확정 항목 서술만 LLM 승격

4단계에서 LLM 이 건드리지 않는 것: 조항 본문(설계 엔진이 이미 냈다), 참조 법령(확인된 상수다),
금액과 문항 번호. 조문 번호를 생성하게 두면 지어낸 인용이 나온다.
