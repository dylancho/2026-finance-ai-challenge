...
`buildReferral()` 은 두 입력으로 `mode: "contract" | "petition"` 을 정한다.

```ts
const delegated = !actsAlone(p) || opts.gate?.fired === true;
```

① 설문에서 이미 진단을 받았다고 답한 경우, 또는 ② **2조건 게이트가 발동한 경우**
(AI 경보 + 최근 1개월 내 의사 진단서·장기요양보험 등급 발행서).

`reading.band === "alert"` 를 직접 보지 않는다. 그러면 회의에서 확정한 2조건 게이트를
우회하게 되고, **AI 경보만으로 사람의 법적 지위를 바꾸는** 셈이 된다. 게이트 판정은
`evaluateTrigger()` 가 이미 하고 있으므로 그 결과만 읽는다.

의뢰서에는 탐지 기록이 소명 자료로 실린다 — 점수, 구간, 베이스라인에서 벗어난 신호,
의료 증빙의 종류와 발행일. 법정후견 청구에 실제로 필요한 것이 이것이다. 이탈이 없는 신호는
싣지 않는다. 전부 나열하면 소명이 아니라 목록이 된다.

**문서 어디에도 인지장애·치매 여부를 쓰지 않는다.** "평소 금융 패턴과 달라진 지점" 까지만
쓰고, 의학적 판정은 첨부된 진단서에 맡긴다 (선행 스펙 §8).
